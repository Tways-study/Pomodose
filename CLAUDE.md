...
**Auth and goal persistence run on Convex** (`convex/`), not localStorage. `convex/schema.ts` defines `goals` (indexed `by_user_and_date`) alongside `@convex-dev/auth`'s `authTables`. `convex/auth.ts` configures a **password** provider — no email verification on signUp/signIn, so accounts are created and signed in immediately (server-side email format re-validation happens in `validateEmail` since the mutation is reachable directly, bypassing the client's `<input type="email">`). Password reset is scaffolded via `convex/BrevoOTPPasswordReset.ts` (a Convex Auth `Email` provider passed as `Password`'s `reset` option, calling Brevo's transactional email API directly over `fetch()` — `POST https://api.brevo.com/v3/smtp/email` — to email a short numeric OTP) — it's wired end-to-end through `Password`'s native `"reset"`/`"reset-verification"` flows, but reads `BREVO_API_KEY`/`BREVO_SENDER_EMAIL` lazily and throws the distinct `"Password reset is not configured yet."` error if either is unset on the deployment yet (`npx convex env set BREVO_API_KEY <key>` etc. once a real Brevo API key and verified sender email exist). `convex/goals.ts` mutations/queries all call `getAuthedUserId` (`convex/lib/auth.ts`) and check row ownership before mutating. **`convex/_generated/` is committed to git** (see the "Commit convex/_generated/" commit) so CI can typecheck Convex code without running `convex dev`/`codegen` first — regenerate and commit it after schema/function changes. Client-side, `app/ConvexClientProvider.tsx` wraps the app in `ConvexAuthNextjsProvider`, and `proxy.ts` redirects unauthenticated requests to `/login` (JSON 401 for `/api/*`).

**State lives in `app/page.tsx` and flows down, except goals.** The timer is a `useReducer` over `lib/timer-machine.ts`, held only in page state — `dailyDoses`/`focusCycle`/phase are **not** persisted anywhere and reset on reload. Goals are a Convex live query owned by `components/goal-list.tsx` (and independently re-queried by `components/dosey-chat.tsx` for chat context); `goal-list.tsx` reports aggregate done/total counts up to the page via an `onProgressChange` callback. There is no global store for timer/goal state — if a component needs timer state, it must be passed as props from the page; goal state is fetched directly wherever it's needed via `useQuery(api.goals.*)`. The one exception is `components/address-term-provider.tsx`, a context that picks a random address term (e.g. "Pill Whisperer") for how the app refers to the user; it defaults to a deterministic index-0 value and randomizes only in a post-mount `useEffect` to avoid a server/client hydration mismatch — follow that pattern for any future randomized-on-load UI.
...
**The vial timer (`components/vial-timer.tsx`) is the signature UI element** — an SVG vial with a clipped liquid `<rect>` animated via Framer Motion, where `remaining/total` drives the liquid height between fixed `VIAL_TOP`/`VIAL_BOTTOM` coordinates. It is intentionally not a circular progress ring; do not replace it with one. The vial's liquid color is fixed lilac from Tailwind tokens and is **not** driven by `PHASE_ACCENT`.

**Running-state visual chrome is layered and phase-linked.** `lib/phase-theme.ts` exports `PHASE_ACCENT: Record<Phase, { base, deep, tint }>` — the single source of truth for dynamic accent colors while a session runs. `app/page.tsx` computes: two animated radial-gradient washes (bottom-left + top-right, different breathe/drift cadences so they never move in lockstep), a `BubbleField` (rising bubbles, hidden entirely under `prefers-reduced-motion` — a frozen bubble field reads as a glitch, not reduced motion), a header border recolor + blurred under-glow, and card box-shadow rings. All new running-state chrome must pull from `PHASE_ACCENT`, not hardcode hex. Focus uses lilac (`#C9B6E4`); short and long breaks share amber (`#D9B36B`).

**Completion alert and tab title are side-effectful modules that must be explicitly stopped.** `lib/chime.ts` exports `startCompletionAlert()` (rings a bell immediately, then every 4 s) and `stopCompletionAlert()` (must be called whenever the user takes any action: phase tab switch, timer reset, starting a new session — `PhaseTabs`'s `onChange` already calls it). `lib/document-title.ts` manages three states: live countdown via `setRunningTitle(phase, remaining)` on every tick, `flashCompletionTitle()` on session end (flashes the tab title only when the tab is hidden), and `resetTitle()` on idle. Both modules guard `typeof document === "undefined"` so they're SSR-safe.
